...
class refreshMain(APIView):
    ...
    def generate_top_50(req):
        ...
        response = requests.get(
            'https://api-v2.soundcloud.com/search/tracks?q=*&filter.genre_or_tag=hardstyle&sort=popular&client_id=w2Cs8NzMrJqhjiCIinZ1xxNBqPNgTVIe&limit=50&offset=0&linked_partitioning=1&app_version=1689322736&app_locale=en',
            headers=headers,
        )
        dt = response.json()
        current_chart = [{"current_position":index+1,"title":i["title"],"link":i["permalink_url"]} for index,i in enumerate(dt["collection"])][:50]
        ...
        for i, song in enumerate(current_chart, start=1):
            ...
            if song_title in song_positions:
                ...
                chart_obj = Chart(title=song_title, previous_position=None, current_position= curr_pos,indicator= None ,link = link ,spot_name= spot_name, spot_url = spot_url,comp_name=comp_name,comp_url=comp_url)
                chart_obj.save()
